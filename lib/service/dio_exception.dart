    ...
    switch (dioError.type) {
      ...
      case DioExceptionType.badResponse:
        ...
        if (response != null && response is Map) {
          if (dioError.response?.data?["error_class"] == 'Unauthorized') {
            message = "Your session has expired. Please login again.";
          } else if (dioError.response?.data?["translation"] == "Invalid or expired token: user") {
            message = "Your session has expired. Please login again.";
          } else {
            message = dioError.response?.data?["translation"] ??
                dioError.response?.data['message'] ??
                dioError.response?.data?['body']["message"];
          }
